...
public class DrawAnimation implements DroneAnimation{
	...
	@Override
	public void droneToItem(double xPos, double yPos, double itemWidth, double itemLength, double dronePixelX, double dronePixelY)  {
		if (drone1 == null) {
			...
			double xDestination = xPos + (0.5 * itemLength);
			
			double yDestination = yPos + (0.5 * itemWidth);
	...
	public void FarmScan(double farmWidth, double farmLength, double dronePixelX, double dronePixelY) {
		if (drone1 == null) {
			Alert alert = new Alert(Alert.AlertType.ERROR);
			alert.setTitle("Cannot Perform That Action");
			alert.setContentText("No drone exists. Add a drone to the command center and try again.");
