...
public class View {
	...
	public void setRoot() {
		if (farmList.getRoot() == null) {
			...
			root.setName("Root");
			root.setWidth(800); // 800
			root.setLength(600); // 600
...
//			System.out.println((addChild.getValue().getHeight()) / 2);
	...
	public void addCommandCenter() {
		...
		if (farmList.getRoot() == null) {
			...
			commandCenter.setXPosition(0);
			commandCenter.setYPosition(0);
...
////		FarmScan();
//		DrawAnimation scanningFarm = new DrawAnimation(drone,drone1);
//		scanningFarm.FarmScan(600, 800,0,0);
//	}

	
	public void LaunchSimulator() throws IOException, InterruptedException {
		DrawAnimation draw = new DrawAnimation(drone,drone1);
		double xPos = Double.parseDouble(xPositionField.getText());
		double yPos = Double.parseDouble(yPositionField.getText());
		double width = Double.parseDouble(widthField.getText());
		double length = Double.parseDouble(lengthField.getText());
		
    	if (this.scanOneToggleGroup.getSelectedToggle().equals(this.scanFarmRadioButton)) {
    		draw.FarmScan(600, 800,0,0);
    	}
   	      
    	if (this.scanOneToggleGroup.getSelectedToggle().equals(this.scanItemRadioButton)) {
    		draw.droneToItem(xPos, yPos, width, length, drone.getXPosition(), drone.getYPosition());
    	}

	}

	public void LaunchDrone() throws IOException, InterruptedException {
		double farmWidth = farmList.getRoot().getValue().getWidth();
		double farmLength = farmList.getRoot().getValue().getLength();
		double dronePixelX = drone.getXPosition();
		double dronePixelY = drone.getYPosition(); 
		double xPos = Double.parseDouble(xPositionField.getText());
		double yPos = Double.parseDouble(yPositionField.getText());
		double width = Double.parseDouble(widthField.getText());
		double length = Double.parseDouble(lengthField.getText()); 
		TelloDrone td = new TelloDrone();
		TelloAdapter tello = new TelloAdapter(td);
		
		if (this.scanOneToggleGroup.getSelectedToggle().equals(this.scanFarmRadioButton)) {
			tello.FarmScan(farmWidth, farmLength, dronePixelX, dronePixelY);
			
    	}
   	      
    	if (this.scanOneToggleGroup.getSelectedToggle().equals(this.scanItemRadioButton)) {
    		tello.droneToItem(xPos, yPos, width, length, drone.getXPosition(), drone.getYPosition());
